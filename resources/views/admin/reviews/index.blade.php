                        ...
                        <div class="page-title-box">
                            @can('create reviews')
                                <a class="btn btn-success"
                                    href="{{ route('admin.reviews.create') }}">{{ __('attributes.create') }}</a>
                            @endcan
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.reviews') }}</h4>
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $reviews)
                                                <tr id="row-{{ $reviews->id ?? '' }}">
                                                    <td>{{ $loop->iteration ?? '' }}</td>
                                                    <td><img src="{{ App\Helpers\Image::getMediaUrl($reviews, 'reviews') }}"
                                                            alt="reviews" width="100"
                                                            onclick="openImage('{{ App\Helpers\Image::getMediaUrl($reviews, 'reviews') }}')"
                                                    ...
                                                    <td>{{ shortenText($reviews->name ?? '', 10) }}</td>
                                                    <td>{{ shortenText($reviews->title ?? '', 10) }}</td>
                                                    <td>{{ shortenText($reviews->description ?? '', 10) }}</td>
                                                    <td>
                                                        @can('active reviews')
                                                            <div class="form-check form-switch">
                                                                <input class="form-check-input" type="checkbox" name="status"
                                                                    id="active-{{ $reviews->id }}"
                                                                    @if ($reviews->active == 1) checked @endif
                                                                    data-id="{{ $reviews->id }}">
                                                                <label class="form-check-label"
                                                                    for="active-{{ $reviews->id }}"></label>
                                                            </div>
                                                        @endcan
                                                    </td>
                                                    <td>
                                                        ...
                                                        @can('delete reviews')
                                                            <button type="button" class="btn btn-danger btn-block btn-delete"
                                                                ...
                                                                data-bs-target="#delete{{ $reviews->id }}">
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="delete{{ $reviews->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $reviews->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $reviews->id }}"
                                                                action="{{ route('admin.reviews.delete', $reviews->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.reviews.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
