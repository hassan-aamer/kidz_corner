...
@section('title', __('attributes.banners'))
                    ...
                    <div class="col-12">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <h4 class="header-title mt-0 mb-1">{{ __('attributes.banners') }}</h4>
                                    ...
                                    <tbody>
                                        @if ($result->count())
                                            @foreach ($result as $banners)
                                                ...
                                                <!-- Delete Modal -->
                                                <div class="modal fade" id="delete{{ $banners->id }}" tabindex="-1"
                                                            ...
                                                            <div class="modal-header">
                                                                <h4 class="modal-title" id="myCenterModalLabel">
                                                                    @lang('attributes.delete') : <span
                                                                        class="text-danger">{{ $banners->title }}</span>
                                                                </h4>
                                                                <button type="button" class="btn-close"
                                                                    data-bs-dismiss="modal" aria-label="Close"></button>
                                                            </div>
                                                            <form id="deleteForm{{ $banners->id }}"
                                                                action="{{ route('admin.banners.delete', $banners->id) }}"
                                                                method="post">
                                                                @csrf
                                                                @method('delete')
    ...
    <script>
        document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                fetch("{{ route('admin.banners.status', app()->getLocale()) }}", {
                    method: 'POST',
                    headers: {
